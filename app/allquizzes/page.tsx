...
const AllContractsPage = dynamic(() => Promise.resolve(() => {
 ...
 fetch(`https://dre-2.warp.cc/contract?id=wk4ZWf6v5CY5o5-pjfkO7b1ezIkkGMXIAfJjchPf3bY&query=$.trivias.${window?.walletAddress}`).then(res=>res.json()).then(r=>{
  let fetchedContractIds=r.result[0]
  Promise.all(fetchedContractIds.map(async fcid=>{
    console.log(fcid)
    return await fetchContractData(fcid)
  })).then(fetchedContracts=>{
    console.log("Fetched contracts for "+window.walletAddress+", ",fetchedContracts)
